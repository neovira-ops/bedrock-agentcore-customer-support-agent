...
class MemoryHook(HookProvider):
    ...
    def retrieve_customer_context(self, event: MessageAddedEvent):
        ...
        logger.info(
            "MEMORY RETRIEVAL: actor=%s session=%s query=%r",
            self.actor_id,
            self.session_id,
            query,
        )
        ...
        if memories_found:
            logger.info("MEMORY RETRIEVAL FOUND: %s", memories_found)
            ...
            logger.info("MEMORY RETRIEVAL FOUND: none")
    ...
    try:
        ...
        with mcp_client:
            try:
                ...
                return (
                    "Sorry, the customer support Gateway timed out. "
                    "Please try again."
                )
            except ConnectionError:
                logger.exception("Gateway connection failed")
                return (
                    "Sorry, I could not connect to the customer support "
                ...
                return (
                    "Sorry, the customer support Gateway is currently "
